...
`endif //
    ...
    // Operation delays
    //--------------------
// all of these freaking times are AMAZINGLY wrong
// I mean, seriously, it's like they don't know what units
// they're using in some cases, and in others they're just
// wrong for the MT25QL128.
// THANKS MICRON
   `ifdef PowDown
`ifdef MT25QU128ABB8E0
...
`endif
    `endif
`ifdef MT25QU128ABB8E0
    parameter time tPP = 1e6;  // whatever, leave this the same, it's wrong but I don't care
    parameter time tSSE = 5e6; // 5 ms not 3 SECONDS
    parameter time t32SSE = 100e6; // 100 ms not 3 SECONDS
    parameter time tSE = 150e6; // 150 ms not 700 ms (seriously, tSSE > tSE? WTF?)
    parameter time tBE = 38e9; // 38 seconds not 240, not like we'll ever try this in simulation
`elsif N25Q064A13xx4My
